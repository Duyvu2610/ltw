...
<path
					...
					<div
						class="flex justify-between items-center md:absolute md:w-full md:top-6">
						<img alt="logo" src="${flight.airline().imagePath() }"
							class="w-20 h-auto">
						<div
							class="flex flex-col pt-2 pb-6 justify-center items-center gap-2">

							<span class="font-bold text-xl text-[#ff5e1f] select-none"><fmt:formatNumber
									value="${flight.price()}" pattern="#,##0" /> VND</span>
							<form action="flights?id=${flight.id()}" method="post">
								<button class="px-6 py-2 bg-[#0194f3] text-white rounded-full">Book Now</button>
							</form>
						</div>
					</div>
				</div>
		...
		<button type="button"
			class="py-2 px-4 flex justify-center items-center my-6 bg-white hover:bg-[#777E90] md:mx-auto md:w-[140px] w-full transition ease-in duration-200 text-center text-base font-semibold border border-black focus:outline-none focus:ring-2 focus:ring-offset-2  rounded-xl max-w-md">
			<svg width="20" height="20" fill="currentColor"
				class="mr-2 animate-spin" viewBox="0 0 1792 1792"
